...
public:
    ...
    PcieTransferResults HostHP_N_Readers_Impl(benchmark::State& state) {
        ...
        if (state.range(4) == 1) {  // enable
            hp_duration = HostWriteHP<caching_src_vector>(hp_base, hp_size, src_data, total_size, page_size);
        }
        thread.join();
    ...
    PcieTransferResults Batching_Memcpy_Test(int64_t total_size, int64_t batch_size) {
        PcieTransferResults results;

        auto pages = total_size / batch_size;
        ...
        results.host_hugepage_bytes_processed = total_size;
        results.host_hugepage_writing_duration =
            this->HostWriteHP<false>(hugepage_ptr, hugepage_size, rnd_data, total_size, batch_size);
        return results;
    }
};
...
BENCHMARK_DEFINE_F(MemCpyPcieBench, BM_BatchSizing)(benchmark::State& state) {
    ...
    for (auto _ : state) {
        auto res = this->Batching_Memcpy_Test(total_size, batch_size);
        state.SetIterationTime(res.host_hugepage_writing_duration.count());
    }
    state.SetBytesProcessed(total_size * state.iterations());
    state.counters["total_size"] = total_size;
...
// BENCHMARK_REGISTER_F(MemCpyPcieBench, BM_HostHP_N_Readers)
//     ->Name("Host_Write_HP_N_Readers_WithHostCopy")
//     ->ArgsProduct({
//         /*Total Size*/ {1_GB},
//         /*Page Size*/ {4_KB, 16_KB, 32_KB},
//         /*N Reader Kernels*/ {1, 2, 3, 4, 5, 6, 7, 8, 16, 24, 32},
...
BENCHMARK_REGISTER_F(MemCpyPcieBench, BM_BatchSizing)
    ...
    ->ArgsProduct({
        {1_GB},                                                             // Total size
        {32_KB, 16_KB, 8_KB, 4_KB, 1_KB, 512, 256, 128, 64, 32, 16, 8, 4},  // Batch size
    });
