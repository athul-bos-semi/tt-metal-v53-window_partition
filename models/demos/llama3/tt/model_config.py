...
class LlamaOptimizations:
    ...
    def performance(cls, model_name):
        ...
        """
        return cls({LayerGroup.FF1_FF3: PrecisionSetting.BFP4_LOFI})
...
class TtModelArgs:
    ...
    def __init__(
        ...
        if device is not None:  # Avoid issue with test_llama_torch.py not having a device
            ...
            for layer, precision in optimizations.layer_settings.items():
                dtype, kernel_cfg = precision_setting_lookup[precision]
                self.model_config[f"{layer.value}_DTYPE"] = dtype
                self.model_config[f"{layer.value}_COMPUTE_KERNEL_CFG"] = kernel_cfg
